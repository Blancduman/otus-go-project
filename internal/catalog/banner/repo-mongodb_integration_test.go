...
func (s *MongoDBRepoTestSuite) SetupSuite() {
	...
	s.coll = s.db.Collection("banner", options.Collection())
...
func (s *MongoDBRepoTestSuite) SetupTest() {
	...
	for i, m := range []Banner{Fixture1(), Fixture2(), Fixture3()} {
		docs[i] = bannerDocFromModel(m)
	}
...
func (s *MongoDBRepoTestSuite) Test_Create() {
	...
	id, err := s.repo.Create(context.TODO(), Banner{
		ID:          0,
		Description: "Test 4",
	})
	s.Require().NoError(err)
