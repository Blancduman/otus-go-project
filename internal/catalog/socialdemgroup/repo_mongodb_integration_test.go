...
func (s *MongoDBRepoTestSuite) SetupSuite() {
	...
	s.coll = s.db.Collection("socialdemgroup", options.Collection())
...
func (s *MongoDBRepoTestSuite) SetupTest() {
	...
	for i, m := range []SocialDemGroup{Fixture1(), Fixture2(), Fixture3()} {
		docs[i] = socialDemGroupDocFromModel(m)
	}
...
func (s *MongoDBRepoTestSuite) Test_Create() {
	...
	id, err := s.repo.Create(context.TODO(), SocialDemGroup{
		ID:          0,
		Description: "Test 4",
	})
	s.Require().NoError(err)
